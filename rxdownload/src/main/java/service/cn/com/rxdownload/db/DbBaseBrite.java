...
public class DbBaseBrite<T extends BaseModel> {
    ...
    public DbBaseBrite(Class<T> clazz, SQLCondition[] selectionArgs,BriteDatabase briteDatabase) {
        this.clazz = clazz;
...
//        }
    ...
    public static <T> void updateField(Class<T> clazz,SQLCondition[] setValue ,ConditionGroup conditionsWhere) {

        SQLite.update(clazz).set(setValue).where(conditionsWhere).execute();
    }
